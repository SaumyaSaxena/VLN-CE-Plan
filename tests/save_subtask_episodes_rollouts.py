...
def get_gt_end_steps_for_stepwise_instructions(tstamp_posetrace_idx, pose_trace, gt_data, gt_data_rollouts, episode_id):
    ...
    for i in range(len(tstamp_posetrace_idx)-1):
        poset = np.array(pose_trace['extrinsic_matrix'][tstamp_posetrace_idx[i+1],:3, 3])
        # r = R.from_matrix(pose_trace['extrinsic_matrix'][tstamp_posetrace_idx[i+1],:3,:3])
        # pose_quat = list(r.as_quat())
        pose_quat = quaternion.from_rotation_vector(pose_trace['extrinsic_matrix'][tstamp_posetrace_idx[i+1],:3,:3])
        position_diff = np.linalg.norm(gt_locations[tstamp_gt_locations_idxs[-1]:]-poset, axis=1)
...
if __name__== "__main__":
        ...
        for ep_idx in ep_idx_sub:
            ...
            for step_idx, sub_instr in enumerate(stepwise_instruction_steps):
                ...
                if sub_instr_saved == sub_instr:
                    ...
                    episode_new['start_position'] = gt_data_rollouts[data['episodes'][int(ep_idx)]['episode_id']]['locations'][tstamp_gt_locations_idxs[step_idx]]
                    episode_new['start_rotation'] = gt_data_rollouts[data['episodes'][int(ep_idx)]['episode_id']]['rotations'][tstamp_gt_locations_idxs[step_idx]]
                    episode_new['goals'] = [
                        {'position': gt_data_rollouts[data['episodes'][int(ep_idx)]['episode_id']]['locations'][tstamp_gt_locations_idxs[step_idx+1]],
                        'radius': 3.0}]
                    ...
                    gt_data_new[str(episode_idx_new)]['actions'] = gt_data_rollouts[data['episodes'][int(ep_idx)]['episode_id']]['actions'][
                        tstamp_gt_actions_idxs[step_idx]+1:tstamp_gt_actions_idxs[step_idx+1]+1]
                    gt_data_new[str(episode_idx_new)]['locations'] = gt_data_rollouts[data['episodes'][int(ep_idx)]['episode_id']]['locations'][
    ...
    output_file = f'/home/sax1rng/Projects/VLN-CE-Plan/data/datasets/RxR_VLNCE_v0/{data_type}/{data_type}_{role}_subtasks_rollouts_gt.json.gz'
