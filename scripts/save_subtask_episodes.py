...
if __name__== "__main__":
    ...
    for subtask_file in tqdm(subtask_files):
        ...
        for ep_idx in ep_idx_sub:
            ...
            for step_idx, sub_instr in enumerate(stepwise_instruction_steps):
                ...
                episode_new['start_position'] = [
                    pose_trace['extrinsic_matrix'][tstamp_posetrace_idx[step_idx],0, 3],
                    pose_trace['extrinsic_matrix'][tstamp_posetrace_idx[step_idx],1, 3],
                    pose_trace['extrinsic_matrix'][tstamp_posetrace_idx[step_idx],2, 3]
                    ]
                r = R.from_matrix(pose_trace['extrinsic_matrix'][tstamp_posetrace_idx[step_idx],:3,:3])
                episode_new['start_rotation'] = list(r.as_quat())

                episode_new['goals'] = [
                    {'position': [
                        pose_trace['extrinsic_matrix'][tstamp_posetrace_idx[step_idx+1],0, 3],
                        pose_trace['extrinsic_matrix'][tstamp_posetrace_idx[step_idx+1],1, 3],
                        pose_trace['extrinsic_matrix'][tstamp_posetrace_idx[step_idx+1],2, 3]
                        ],
                    'radius': 3.0}]
                ...
                gt_data_new[str(episode_idx_new)]['actions'] = gt_data[data['episodes'][int(ep_idx)]['episode_id']]['actions'][
                    tstamp_gt_actions_idxs[step_idx]:tstamp_gt_actions_idxs[step_idx+1]+1]
    ...
    output_file = f'/home/sax1rng/Projects/VLN-CE-Plan/data/datasets/RxR_VLNCE_v0/{data_type}/{data_type}_{role}_subtasks.json.gz'
    with gzip.open(output_file, "wt") as f:
        f.write(json.dumps(data_new))
        print("Output file saved at ", output_file)
    
    print(f'Saving new GT {data_type} file')
    output_file = f'/home/sax1rng/Projects/VLN-CE-Plan/data/datasets/RxR_VLNCE_v0/{data_type}/{data_type}_{role}_subtasks_gt.json.gz'
    with gzip.open(output_file, "wt") as f:
